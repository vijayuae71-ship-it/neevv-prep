...
import {
  Send, Loader2, Lightbulb, Briefcase, AlertTriangle,
  Layout, Timer, Mic, MicOff,
} from 'lucide-react';
import { isDeepgramAvailable, startRecording, stopRecording, cancelRecording, hasActiveRecording } from '../utils/deepgramSTT';
...
interface DomainChatInterfaceProps {
...
export const DomainChatInterface: React.FC<DomainChatInterfaceProps> = ({
  config,
  messages,
  onSend,
  onHint,
  ...
  const domainLabel = config.domain.charAt(0).toUpperCase() + config.domain.slice(1);
  ...
  const toggleVoice = useCallback(async () => {
    if (isListening) {
      ...
      if (hasActiveRecording()) {
        try {
          ...
          if (transcript) setPendingTranscript(transcript);
        } catch (err) { console.error('Voice stop error:', err); }
    ...
    if (isDeepgramAvailable()) {
      try { await startRecording(); setIsListening(true); return; } catch (err) {
        console.warn('Deepgram failed, falling back:', err);
      }
    }
    ...
    recognition.onresult = (event: any) => {
      ...
      if (finalT.trim()) setPendingTranscript(finalT.trim());
    };
  ...
  useEffect(() => {
    return () => { if (recognitionRef.current) recognitionRef.current.abort(); cancelRecording(); };
  }, []);
    ...
    return (
      <div key={idx} className={`chat ${isCoach ? 'chat-start' : 'chat-end'}`}>
        <div className="chat-header text-xs opacity-70 mb-1">
          {isCoach ? '🤖 Domain Coach' : '👤 You'}
        </div>
        <div className={`chat-bubble max-w-[85%] ${isCoach ? 'bg-base-200 text-base-content' : 'bg-primary text-primary-content'}`}>
    ...
    <div className="flex flex-col h-screen bg-base-200">
      ...
      <div className="navbar bg-base-100 shadow-md px-4">
        <div className="flex-1 gap-2">
          ...
          <span className="badge badge-ghost badge-sm">{domainLabel}</span>
        </div>
        <div className="flex-none flex items-center gap-2">
          <button
            ...
            onClick={() => {
              ...
              if (timerEnabled) {
                if (answerTimerRef.current) { clearInterval(answerTimerRef.current); answerTimerRef.current = null; }
                setTimerActive(false); setTimerExpired(false); setTimerSeconds(120);
              }
            }}
            title={timerEnabled ? 'Disable timer' : 'Enable 2-min timer'}
          >
        ...
        <div className="alert alert-info mx-4 mt-2 shadow-sm">
          ...
          <div className="flex gap-1">
            <button className="btn btn-success btn-xs" onClick={() => {
              if (pendingTranscript.trim()) onSend(pendingTranscript);
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2">
        ...
        {isCoachTyping && !isStreaming && (
          <div className="chat chat-start">
            <div className="chat-header text-xs opacity-70 mb-1">🤖 Domain Coach</div>
